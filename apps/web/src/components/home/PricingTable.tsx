import { PRICING_CONTENT } from "../../constants/home.constants";
  ...
  return (
    <section className="py-20 gsap-reveal">
      <div className="text-left mb-12">
        ...
        <h2 className="text-3xl font-extrabold mt-1">{PRICING_CONTENT.title}</h2>
        <p className={`text-sm mt-1 max-w-xl ${isDark ? "text-zinc-400" : "text-zinc-600"}`}>
          {PRICING_CONTENT.description}
        </p>
      </div>

      <div className={`rounded-[4px] border overflow-hidden ${isDark ? "bg-[#0a0a0c] border-zinc-800" : "bg-white border-zinc-300 shadow-sm"}`}>
          ...
          <table className="w-full text-left text-xs font-mono">
            <thead>
              <tr className={`border-b ${isDark ? "border-zinc-800 bg-black/40 text-zinc-400" : "border-zinc-200 bg-zinc-50 text-zinc-600"}`}>
                {PRICING_CONTENT.headers.map((header, idx) => (
                  <th key={idx} className="py-4 px-6 font-semibold">{header}</th>
                ))}
              </tr>
            </thead>
            <tbody className={`divide-y ${isDark ? "divide-zinc-800/60" : "divide-zinc-200"}`}>
              {PRICING_CONTENT.rows.map((row, i) => (
