  ...
  return (
    <>
      <div className="fixed inset-0 z-[200] bg-dark-bg pointer-events-none opacity-0 transition-opacity duration-500 grid place-items-center">
        <span className="w-8 h-8 border-2 border-solid border-indigo-400 border-t-indigo-500/25 border-r-indigo-500/25 rounded-full opacity-0 animate-nav-glow"></span>
      </div>
      ...
      <nav className="fixed top-0 left-0 right-0 z-[120] h-16 backdrop-blur-md bg-dark-bg/75 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            <button className="flex items-center gap-2 cursor-pointer text-zinc-400 text-[12.5px] bg-white/5 border border-white/10 px-2.5 py-1.5 rounded-lg hover:bg-white/10 transition-colors">
              <Search className="w-3.5 h-3.5" />
              <span>Search</span>
              <kbd className="font-mono text-[10.5px] bg-white/5 border border-white/10 rounded px-1 text-zinc-500">⌘K</kbd>
            </button>

            <a href="https://github.com/RahulDew/prooflog" target="_blank" rel="noreferrer" className="hidden sm:flex items-center gap-2 text-zinc-200 text-[13px] font-medium px-3 py-1.5 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 transition-colors">
